  ...
  private def checkAllReadinessPass(
    ...
    InvariantResult(
      ...
      detail = if (readiness.isEmpty)
        ...
        Some(s"Failed: ${failed.map(r => s"${r.name} (status=${r.statusCode.getOrElse("none")}, expected=${r.expectedStatus})").mkString(", ")}")
    )
  }
