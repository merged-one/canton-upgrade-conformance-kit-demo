...
object ScenarioLoader {
  ...
  def load(path: Path): Either[String, Scenario] = {
    if (!Files.exists(path))
      return Left(s"Scenario file not found: $path")

    Try {
      val content = new String(Files.readAllBytes(path), "UTF-8")
      val yaml = new Yaml()
      val doc = yaml.load(content).asInstanceOf[java.util.Map[String, Any]]
      parseScenario(doc)
  ...
  private def parseScenario(doc: java.util.Map[String, Any]): Either[String, Scenario] = {
    val scenarioMap = Option(doc.get("scenario"))
      .collect { case m: java.util.Map[_, _] => m.asInstanceOf[java.util.Map[String, Any]] }
      .getOrElse(return Left("Missing top-level 'scenario' key"))
    ...
    val endpoints = getList(scenarioMap, "endpoints").map { items =>
      items.flatMap {
    ...
    val invariants = getList(scenarioMap, "invariants").map { items =>
      items.flatMap {
        case m: java.util.Map[_, _] =>
